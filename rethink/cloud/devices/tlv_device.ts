...
export default class TLVDevice extends HADevice {
    ...
    addField(config: Config, options: FieldDefinition, autoreg?: boolean) {
        ...
        if (options.name) this.fields_by_ha[options.name] = options

        if (autoreg !== false) {
            if (options.writable === false) config[options.name + '_topic'] = '$this/' + options.name
            else {
                if (options.readable !== false) config[options.name + '_state_topic'] = '$this/' + options.name
    ...
    processKeyValue(k: number, v: number) {
        ...
        if (def.read_callback) def.read_callback(processed)
        else {
